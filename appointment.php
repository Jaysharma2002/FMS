<?php
session_start();
$did = $_SESSION['did']; // Doctor ID stored in the session
...
if (isset($_POST['appointments'])) {
    ...
    foreach ($appointment_ids as $aid) {
        ...
        $update_query = pg_query($db, "UPDATE appointment SET dstatus = 'Completed' WHERE aid = '$aid' AND did = '$did'") or die("Update query failed");
    }
}

// Query to retrieve the doctor's appointments with status 'Booked'
$query = pg_query($db, "SELECT * FROM appointment WHERE did = '$did' AND dstatus = 'booked'") or die("Cannot Select");
...
if (pg_num_rows($query) > 0) {
    ...
    while ($row = pg_fetch_assoc($query)) {
        echo "<tr>
                ...
                <td style='padding: 10px; border: 1px solid #dddddd;'>{$row['dstatus']}</td>
              </tr>";
    }
    ...
    echo "<center><h3>No booked appointments found for this doctor.</h3></center>";
}

// Close the database connection
pg_close($db);
?>
<a href="doctor_page.html" style="display: block; text-align: center; margin-top: 20px; color: blue; text-decoration: none;">Go Back</a>
